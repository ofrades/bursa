...
const ANALYSIS_FILTERS = ["all", "saved", "watching"] as const;
...
export const Route = createFileRoute("/")({
  validateSearch: (search) => {
    const filter = ANALYSIS_FILTERS.find((f) => f === search.filter);
    if (!filter) return {};
    return { filter };
  },
  beforeLoad: ({ context, search }) => {
    if (!context.session || search.filter) return;
    throw redirect({
      to: "/",
      search: { filter: "watching" },
      replace: true,
    });
  },
...
function LandingHome({ analyses: initialAnalyses }: { analyses: SharedAnalysisRow[] }) {
  const [searchQuery, setSearchQuery] = useState("");

  const queryClient = useQueryClient();
  const metricSymbols = useMemo(
    () => uniqueSymbols(initialAnalyses.map((row) => row.symbol)),
    [initialAnalyses],
  );
  ...
  const analyses = initialAnalyses.map((row) => ({
    ...row,
    ...
    peNtm: metricsBySymbol.get(row.symbol)?.forwardPe ?? null,
    revenueGrowthYoy: metricsBySymbol.get(row.symbol)?.revenueGrowthYoy ?? null,
  }));
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div className="min-w-0 flex-1 lg:max-w-xl">
                  <StockSearchBar
                    ...
                    existingSymbols={analyses.map((row) => row.symbol)}
                    mode="guest"
                    maxWidth="100%"
