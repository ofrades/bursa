...
type WeeklyTrend = "uptrend" | "downtrend" | "sideways";
type CycleTimeframe = "SHORT" | "MEDIUM" | "LONG";
type WeeklySignal = "BUY" | "SELL";
type WeeklyCall = "BUY" | "SELL" | "WAIT";
type WeeklyRiskLevel = "LOW" | "MEDIUM" | "HIGH";
...
export function parseStockThesis(value: string | null | undefined): StockThesis | null {
  ...
  try {
    ...
    const thesis = parsed as StockThesis;
    thesis.confidence = {
      base: normalizeScore(thesis.confidence?.base),
      adjusted: normalizeScore(thesis.confidence?.adjusted),
      delta: 0,
    };
    thesis.confidence.delta =
      thesis.confidence.base != null && thesis.confidence.adjusted != null
        ? thesis.confidence.adjusted - thesis.confidence.base
        : 0;
    return thesis;
  } catch {
    return null;
  ...
  if (weekly.weeklyCall === "WAIT") {
    return {
      ...
      summary:
        ...
        "This week's action is patience, even if the directional case has not fully broken.",
    };
  }

  if (weekly.weeklyCall === "SELL") {
    return {
      title: "Entry window",
      value: "Trim / avoid",
      tone: "cautious",
      summary:
        weekly.weeklyOutlook ||
        "This week's action is defensive, so do not press new exposure here.",
    };
  }
