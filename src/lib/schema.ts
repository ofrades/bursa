...
export const user = sqliteTable("user", {
  ...
  analysisReservedCents: integer("analysis_reserved_cents").notNull().default(0),
...
export const usageLog = sqliteTable(
  "usage_log",
  {
    ...
    usageReported: integer("usage_reported", { mode: "boolean" }).notNull().default(true),
    createdAt: integer("created_at", { mode: "timestamp" }).notNull().$defaultFn(now),
  },
  (t) => [
    unique("uq_usage_log_reservation").on(t.reservationId),
    index("idx_usage_log_user").on(t.userId),
    index("idx_usage_log_symbol").on(t.symbol),
  ],
);
