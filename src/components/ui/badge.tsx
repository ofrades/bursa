...
type SignalKey = Signal | "STRONG_BUY" | "STRONG_SELL";
...
function signalLabelFor(signal: string): string {
  return signal in signalLabel ? signalLabel[signal as SignalKey] : signal.replaceAll("_", " ");
}

function signalClassesFor(signal: string): string {
  return signal in signalBadgeClasses
    ? signalBadgeClasses[signal as SignalKey]
    : signalBadgeClasses.HOLD;
}

function SignalBadge({ signal, className }: { signal: string; className?: string }) {
  const classes = signalClassesFor(signal);
  const label = signalLabelFor(signal);
  return (
...
function StanceBadge({
  ...
  const shortLabel = signalLabelFor(signal);
  const longLabel = { Own: "Own", "Maybe own": "Watch", Avoid: "Avoid" };
...
function LongTermBadge({ stance, className }: { stance: string; className?: string }) {
  const classes =
    stance in longTermBadgeClasses
      ? longTermBadgeClasses[stance as StanceLongTerm]
      : "border-border bg-muted text-muted-foreground";
  const label =
    stance in longTermLabel ? longTermLabel[stance as StanceLongTerm] : stance.toUpperCase();
  return (
    <Badge variant="outline" className={cn(classes, className)}>
      {label}
    </Badge>
...
const supervisorSeverityClasses = {
