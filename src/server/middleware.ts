...
export const authMiddleware = createMiddleware({ type: 'function' }).server(
  async ({ next }) => {
    ...
    if (!session) return next({ context: { session: null, analysisCredits: 0, isAdmin: false } as any })
    ...
    const [existing] = await db.select({ analysisCredits: user.analysisCredits })
    ...
    return next({ context: { session, analysisCredits: isAdmin ? 999999 : (existing?.analysisCredits ?? 0), isAdmin } as any })
  },
)
