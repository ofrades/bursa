...
const output = {
  ...
  endpoints: endpoints.map((ep: any) => ({
    ...
    pricing_per_million_tokens: {
      ...
      cached_input: ep.pricing?.input_cache_read
        ? `$${(parseFloat(ep.pricing.input_cache_read) * 1_000_000).toFixed(2)}`
        : undefined,
      discount: ep.pricing?.discount ? `${(ep.pricing.discount * 100).toFixed(0)}%` : undefined,
    },
