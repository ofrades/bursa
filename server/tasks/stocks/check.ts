...
export default defineTask({
  ...
  async run() {
    ...
    const [
      { getDb },
      { sql, eq, and },
      { watchlist, weeklyRecommendation, dailyUpdate, stockMemory },
    ] = await Promise.all([
      import("../../../src/lib/db"),
      import("drizzle-orm"),
    ...
    const items = await db
      .select({ userId: watchlist.userId, symbol: watchlist.symbol })
      .from(watchlist);
    ...
    for (const item of items) {
      try {
        const [quote, summary] = await Promise.all([
          ...
          (yf.quoteSummary(item.symbol, { modules: ["calendarEvents"] }) as Promise<any>).catch(
            () => null,
          ),
        ]);
        ...
        const [rec] = await db
          ...
          .where(
            and(
              eq(weeklyRecommendation.userId, item.userId),
              eq(weeklyRecommendation.symbol, item.symbol),
